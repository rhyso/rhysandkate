function reInit(target){
    ...
    $(window).scroll(function() {
        if ($(window).scrollTop() > 0) {
            $(target+' nav').addClass('scrolled');
        } else {
            $(target+' nav').removeClass('scrolled');
        }
    });

    // Set nav container height for fixed nav

    if (!$(target+' nav').hasClass('transparent')) {
    ...
    $(target+' .mobile-toggle').click(function() {
        $(target+' nav').toggleClass('nav-open');
    });
    ...
    setTimeout(function() {
        ...
        $(target+' .header.fadeContent').each(function() {
            $(this).addClass('fadeIn');
        });
    }, 200);
    ...
    if (!(/Android|iPhone|iPad|iPod|BlackBerry|Windows Phone/i).test(navigator.userAgent || navigator.vendor || window.opera)) {
        ...
        $(target+' .parallax').removeClass('parallax');
    }

    // Image fade on story 2 element

    $(target+' .story-2 img').mouseenter(function() {
        $(this).removeClass('fade');
        $(this).siblings().addClass('fade');
    });

    $(target+' .story-2 img').mouseleave(function() {
    ...
    $(target+' .radio-holder').click(function() {
